...
def decode_wav_bytes(wav_bytes: bytes) -> tuple[np.ndarray, int]:
    ...
    try:
        ...
        try:
            ...
            # Fallback also failed - raise the ORIGINAL soundfile error,
            # since that's the more informative one (the base64 error is
            # usually just "invalid padding" or similar, which doesn't tell
            # you the real problem is the audio format itself).
...
if __name__ == "__main__":
    # Diagnostic: pull the first 3 samples for each of the 5 languages
    # (scripted train split only, to keep this fast) so Teammates 1 and 2
    # can audit real output across every remaining language at once.
